...
class EvaluateTest(unittest.TestCase):
    ...
    def test_sensitivity_of(self):
        res = sensitivity_of('RF01', clusters, names_by_family)
        self.assertAlmostEqual(res, 1.0)
        res = sensitivity_of('RF02', clusters, names_by_family)
        self.assertAlmostEqual(res, 0.5)

    def test_precision_of(self):
        res = precision_of('RF01', clusters)
        self.assertAlmostEqual(res, 0.8)
        res = precision_of('RF02', clusters)
        self.assertAlmostEqual(res, 1.0)
        res = precision_of('RF03', clusters)
    ...
    def test_max_in_cluster(self):
        res = max_in_cluster_of('RF01', clusters, names_by_family)
        self.assertAlmostEqual(res, 0.5)
        res = max_in_cluster_of('RF02', clusters, names_by_family)
        self.assertAlmostEqual(res, 0.5)

    def test_nofold_get_name_clusters(self):
        from os.path import join
        path = join(utils.path.queries_path(), 'rfam75id', 'rfam75id.clusters_s3rSpec_top.txt_expanded_merged_bs17.41bgNoneGloc.txt')
