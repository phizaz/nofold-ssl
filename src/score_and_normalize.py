...
if __name__ == '__main__':
    ...
    parser.add_option("--cpus", action="store", type='int', default=1, dest="MAX_CPU",
                      help="Maximum number of CPUs to use. Default is [%default].")
    parser.add_option("--infernal-path", action="store", default=None, dest="INFERNAL_PATH",
                      help="Path to folder containing Infernal executables. Default is to assume they have been added to your PATH and can be called by name.")
    parser.add_option("--cm-folder", action="store", default="../models/rfam_cms/", dest="CM_FOLDER",
                      help="Folder containing the covariance models you would like to use. Default is to use the 1,973 Rfam CMs included in [%default].")
    ...
    print ""
    print "Finished scoring %s sequences on %s CMs in %.2fs / %.4fhr on %s core(s) (%.4fcpu-s / %.4fcpu-hrs, or ~ %.2fs/cm)" % (
    int(float(numLines) / 2), count, elapsedTime, elapsedHrs, NUM_PROCESSES, totalTime, cpuHrs, avgTime)
